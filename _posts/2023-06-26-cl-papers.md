...
The authors define their MDP so that the policy $\pi$ is goal-parametrized, allowing for multi-task learning. So, if a goal state $g \in \mathcal{S}^{g}$ is given, then $ \pi \left( a \text{ , } g \right)$ reflects the probability of the agent taking action $a_{t}$ from state $s_{t}$, given that their goal is to reach state $g$. 
