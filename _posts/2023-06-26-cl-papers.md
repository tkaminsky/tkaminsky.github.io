...
The authors define their MDP so that the policy $\pi$ is goal-parametrized, allowing for multi-task learning. So, if a goal state $g \in \mathcal{S}^{g}$ is given, then $ \pi ( a_t | s_t \text{ , } g)$ reflects the probability of the agent taking action $a_{t}$ from state $s_{t}$, given that their goal is to reach state $g$. 
...
If the agent reaches the goal, the simulation stops, so the total reward is either $0$ or $1$, indicating the agent's success. Thus, the expected value of a policy $\pi$ becomes, by the [fundamental bridge](https://trinhngocthuyen.com/posts/tech/indicator-rvs-and-the-fundamental-bridge/),
