...
* **Task**: For some RL agent $\mathcal{M}_{\theta_{t}}$ parametrized by $\theta_{t}$ in an environment with possible states $\mathcal{S}$, discover:
    1. A set of possible 'goal' states $\mathcal{S}^{g} \subset \mathcal{S}$ which a model $\mathcal{M}_{\theta_{t}}$ can be trained to reach.
...
* **Method**: Use a GAN to learn *Goals of Intermediate Difficulty* at iteration $i$ ($GOID_i$), which have a probability $p_{\text{min}} \leq p \leq p_{\text{max}}$ of success with the current agent. Thus, each goal is selected to be both informative and reasonable, hopefully increasing sample efficiency.
* **Results**: 
